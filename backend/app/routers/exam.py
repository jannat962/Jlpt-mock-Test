...
def get_user_metrics(current_user: int = Depends(get_current_user), db: Session = Depends(get_db)):
    """Return learner or teacher metric counts for the current user."""
    user = db.query(models.User).filter(models.User.id == current_user).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")

    available_tests = db.query(models.MockTest).count()
    completed_sessions = db.query(models.TestSession).filter(
        models.TestSession.user_id == current_user,
        models.TestSession.is_completed == True
    ).count()
    active_sessions = db.query(models.TestSession).filter(
        models.TestSession.user_id == current_user,
        models.TestSession.is_completed == False
    ).count()
    template_count = db.query(models.QuestionTemplate).filter(models.QuestionTemplate.user_id == current_user).count()

    readiness_score = float(user.readiness_score or 0.0)
    mastery_points = int(readiness_score * 10)
    streak_progress = min(100, 40 + completed_sessions * 10)
    growth_goals = max(1, completed_sessions + template_count)
    ...
    return {
        ...
        "ai_badges": template_count,
        "growth_goals": growth_goals,
        "active_courses": available_tests,
...
def submit_test(
    ...
    # Unique-key answers to prevent duplicate scoring exploits (keep the last answer submitted for each question)
    unique_answers = {a.question_id: a.selected_index for a in submission.answers}

    # Fetch all submitted questions at once, scoped to the current test ID to prevent cross-test injection
    submitted_q_ids = list(unique_answers.keys())
    questions = db.query(models.Question).filter(
        models.Question.id.in_(submitted_q_ids),
        models.Question.test_id == session.test_id
    ).all()
    question_map = {q.id: q for q in questions}

    for question_id, selected_index in unique_answers.items():
